(function() {
  var accessToken, el, head, i, initializeRollbar, len, meta;

  initializeRollbar = (function() {
    var initialized;
    initialized = false;
  ...
  if (head = document.getElementsByTagName('head')[0]) {
    ...
    for (i = 0, len = meta.length; i < len; i++) {
      el = meta[i];
      if (el.getAttribute('name') === 'rollbar:access_token') {
        accessToken = el.getAttribute('content');
        break;
      }
    }
    if (accessToken) {
      initializeRollbar(accessToken);
    }
  }
...
(function() {
  ...
  if (!(this.console && typeof this.console === 'object')) {
    this.console = {};
  }
  ...
  for (i = 0, len = ref.length; i < len; i++) {
    prop = ref[i];
    if (typeof this.console[prop] !== 'function') {
      this.console[prop] = function() {};
    }
  }
...
(function() {
  ...
  $(document).on('page:change', function() {
    return $('.js-form .js-submit-action').click(function(e) {
      var $button, $buttons, $errors, $form, $progress, $widget, atLeast, data, extraData, i, k, len, name, now, originalText, progressText, promise, ref, ref1, started, textChanged, url, v, value;
      e.preventDefault();
      $button = $(e.currentTarget);
      $widget = $button.parents('.js-form');
      $form = $widget.find('form');
      if ($widget.data('submitting') || $widget.data('submitted')) {
        return;
      }
      ...
      if (extraData) {
        for (k in extraData) {
          if (!hasProp.call(extraData, k)) continue;
          v = extraData[k];
          data[k.snakeCase()] = v;
        }
      }
      ...
      promise.done(function(res) {
        return atLeast(500, function() {
          ...
          $success.fadeIn(300);
          return typeof $.scrollTo === "function" ? $.scrollTo($success, {
            pad: 65
          }) : void 0;
        });
      });
      promise.fail(function(res) {
        var $group, $input, $label, errors, j, len1, msg, ref2, ref3;
        if (errors = res != null ? (ref2 = res.responseJSON) != null ? ref2.errors : void 0 : void 0) {
          $errors.empty();
          $widget.find('.js-error').remove();
          if ($.isArray(errors)) {
            ref3 = res.responseJSON.errors.slice(0, 2);
            ...
            for (name in errors) {
              if (!hasProp.call(errors, name)) continue;
              msg = errors[name];
              $input = (function() {
                try {
          ...
          return atLeast(500, function() {
            ...
            $failure.fadeIn(300);
            return typeof $.scrollTo === "function" ? $.scrollTo($failure, {
              pad: 65
            }) : void 0;
          });
        }
      });
      atLeast = function(ms, callback) {
        var current, passed;
        current = now();
        passed = current - started;
        if (passed < ms) {
          return setTimeout(callback, ms - passed);
        } else {
          return callback();
        }
      };
      return promise.always(function() {
        ...
        return $widget.find('.js-button').prop('disabled', false);
      });
    });
  });
...
(function() {
  ...
  froalaTables = function() {
    return $('.js-wysiwyg').find('table').each(function() {
      var $cell, $cells, $label, $rows, $table, cell, i, index, label, labels, len, results, row;
      $table = $(this);
      $rows = null;
      labels = [];
      ...
      for (index = i = 0, len = labels.length; i < len; index = ++i) {
        label = labels[index];
        $rows || ($rows = $table.find('> tbody > tr'));
        results.push((function() {
          var j, len1, results1;
          results1 = [];
          for (j = 0, len1 = $rows.length; j < len1; j++) {
            row = $rows[j];
            $cells = $(row).children(index ? "td:gt(" + (index - 1) + ")" : 'td');
            results1.push((function() {
              ...
              for (k = 0, len2 = $cells.length; k < len2; k++) {
                cell = $cells[k];
                $cell = $(cell);
                if (parseInt($cell.attr('colspan')) === labels.length) {
                  continue;
                }
                $label = $(cell).children('.cell-label');
                if ($label[0]) {
                  results2.push($label.text(label));
                } else {
                  results2.push($(cell).html(['<div class="cell-label">', label, '</div>', '<div class="cell-content">', $cell.html(), '</div>', '<div class="clear"></div>'].join('')));
                }
              }
              return results2;
            })());
...
(function() {
  var el, head, i, initializeGoogleAnalytics, len, meta, trackingID;
...
(function() {
  ...
  createGallery = function($photo, $pswp) {
    ...
    $photos.each(function() {
      var $el, obj, preshow, ref, src, value;
      $el = $(this);
      if (src = $el.attr('data-url') || $el.attr('src')) {
        value = $el.attr('data-photoswipe-no-preshow');
        preshow = (value == null) || (value === false || value === 'false');
        obj = {
          $el: $el,
          src: src,
          msrc: preshow ? $el.attr('data-thumb-url') || $el.attr('src') : void 0,
          w: $el.attr('data-width') || $el.attr('width') || $el.prop('naturalWidth') || 800,
          h: $el.attr('data-height') || $el.attr('height') || $el.prop('naturalHeight') || 600,
          title: ((ref = $el.siblings('figcaption')) != null ? typeof ref.text === "function" ? ref.text() : void 0 : void 0) || $el.attr('alt') || $el.attr('data-description')
        };
        return descriptors.push(obj);
      }
    });
    options = {
      history: false,
      ...
      getThumbBoundsFn: (function() {
        ...
        anim = (value == null) || (value === false || value === 'false');
        if (anim) {
          return function(index) {
            var $el, pageYScroll, rect, ref;
            $el = (ref = descriptors[index]) != null ? ref.$el : void 0;
            if ($el != null ? $el[0] : void 0) {
    ...
    return new PhotoSwipe($pswp[0], PhotoSwipeUI_Default, descriptors, options);
  };
  ...
  $(function() {
    $(document).on('click', ['.js-photoswipe-scope img:not([data-no-photoswipe])', '.js-photoswipe-scope .js-photoswipe'].join(','), function(e) {
      var $el, ref;
      $el = $(e.currentTarget);
      if ($el.prop('tagName').toLowerCase() !== 'img') {
        $el = $el.find('img:not([data-no-photoswipe])');
      }
      if ((ref = createGallery($el, $pswp)) != null) {
        ref.init();
      }
    });
    return $(document).on('click', '.js-wysiwyg figure', function(e) {
      var ref;
      if ((ref = createGallery($(e.currentTarget).find('img'), $pswp)) != null) {
        ref.init();
      }
    });
  });
...
(function() {
  ...
  $.scrollTo = function(target, options) {
    var $el, $root, ref, ref1, speed, top;
    if (options == null) {
      options = {};
    }
    $el = $(target);
    ...
    if (options.visible === false || (typeof $el.visible === "function" ? $el.visible() : void 0) === false && $el.is(':visible')) {
      speed = (ref = options.speed) != null ? ref : 500;
      top = (ref1 = options.top) != null ? ref1 : $el.offset().top;
      if (options.pad) {
        top -= options.pad;
      }
      $root = $('html, body');
  ...
  callback = function() {
    var index, match, next, ref, selectors;
    if (match = (ref = location.search) != null ? typeof ref.match === "function" ? ref.match(/scroll=([^&]+)/) : void 0 : void 0) {
      try {
    ...
    return $('.js-scroll-trigger').click(function(e) {
      var $block, selector;
      e.preventDefault();
...
(function() {
  var firstRun, handleCharsetMeta, handleNamedMeta;

  firstRun = true;

  handleNamedMeta = function($head, attrs) {
    var $meta, exists;
  ...
  handleCharsetMeta = function($head, attrs) {
    var $meta, exists;
  ...
  this.turbohead = function(list) {
    var $head, attrs, i, len;
    if (firstRun) {
      firstRun = false;
    } else {
      $head = $('head');
...
(function() {
  $(function() {
    var apiURL, pageURL;
    apiURL = "/api/words/:id";
    pageURL = "/%D0%A1%D0%BB%D0%BE%D0%B2%D0%B0%D1%80%D1%8C/:word_alias";
    return $(document).on('click', '.js-word-toggle', function() {
      var $def, $dic, $guide, $toggle, $word;
      $toggle = $(this);
      $word = $toggle.parents('.js-word');
      $dic = $('#js-dictionary');
      $def = $dic.find('#js-word-definition');
      $guide = $dic.find('#js-dictionary-guide');
      $dic.find('.js-word').removeClass('chosen');
      $word.addClass('chosen');
      $def.find('.js-name').text($toggle.text());
      $def.find('.js-wysiwyg').load(apiURL.replace(':id', $word.data('id')));
      $guide.addClass('hide');
      ...
      try {
        window.history.replaceState({}, document.title, $toggle.attr('href'));
      } catch (undefined) {}
      return false;
    });
  });
...
(function() {
  $(document).on('page:change', function() {
    var $desktopMenu, $desktopMenuItems, $mobileMenu, $mobileMenuItems;
    $desktopMenu = $('.js-desktop-menu');
    $mobileMenu = $('.js-mobile-menu');
    $desktopMenuItems = $desktopMenu.find('.js-desktop-menu-items');
    $mobileMenuItems = $mobileMenu.find('.js-mobile-menu-items');
    $desktopMenuItems.slicknav({
      prependTo: $mobileMenuItems,
      label: '',
      duration: 225
    });
    return $mobileMenu.find('.js-mobile-menu-toggle').click(function() {
      $desktopMenuItems.slicknav('toggle');
      return false;
    });
  });
...
(function() {
  $(document).on('page:change', function() {
    ...
    $('.js-gallery-box-slides').slick($.extend({}, defaults));
    return $('.js-hot-deals-box-slides').slick($.extend({}, defaults, {
      slidesToShow: 1,
      vertical: true
    }));
  });
